...
class Mysqli extends AbstractDb {
    ...
    protected function _connect($params) {
        if (!extension_loaded('mysqli')) {
            throw new Db\Exception('NO_MYSQLI_EXTENSION_FOUND');
        }

        if ($params['persistent']) {
            throw new Db\Exception('MYSQLI_EXTENSTION_DOES_NOT_SUPPORT_PERSISTENT_CONNECTION');
        }

        $this->_connection = mysqli_init();

        $connected = @mysqli_real_connect(
                        $this->_connection, $params['host'], $params['user'], $params['password'], $params['database'], $params['port']
        );
